...
| `ui-bugs.mjs`        | Guards three fixed UI bugs: no duplicate warband skill lists, render survives adding a skill, and the PDF export runs without the `_sheetBytes` binding error. |
| `fallen.mjs`         | Dead (11-15) removes a hero (whole model) or one henchman (group qty-1, group gone at 0) into `S.fallen`, excluded from totals and the PDF; henchmen group by identical exp+equipment; a LIFO undo restores the most recent death. |
| `leader-death.mjs`   | When the leader unit is slain: the "unit required" validation drops, a new one can't be recruited, and the highest-Leadership hero succeeds (XP breaking ties); Undead pass command to the Necromancer or collapse if none. |
| `tts-and-adv.mjs`    | TTS cards show skill text (not `[object Object]`) and include standalone rare items; the "Advance due!" flag compares earned vs applied advances rather than firing on every XP threshold. |
| `compat.mjs`         | A frozen old-format save still loads: missing keys fall back to defaults, round trips preserve data, and every legacy key is still written. |
...
One caveat worth knowing: a save containing fallen warriors, opened in a build
from before that feature, loads its living warriors correctly (dead henchmen are
already decremented) but silently drops the `fallen` record.
